...
def collect_findings(checks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Collect normalized review items while preserving check order."""
    return [finding for check in checks if (finding := finding_from_check(check))]


def write_findings(outdir: Path, checks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Write the machine-readable finding index for a completed audit run."""
    import json

    findings = collect_findings(checks)
    try:
